...
def get_modification_factor(rule: DoseModificationRule) -> float:
    ...
    # Discontinue means omit the drug entirely
    if mod_type == "discontinue":
        return 0.0
    ...
    if "omit" in mod_str or "discontinue" in mod_str:
        return 0.0
...
class ProtocolEngine:
    ...
    def get_protocol(self, code: str) -> Optional[Protocol]:
        """Get protocol by code (case-insensitive, flexible matching)"""
        # Exact match first
        if code in self.protocols:
            return self.protocols[code]

        # Normalized match (remove spaces/hyphens, uppercase)
        code_norm = code.upper().replace(" ", "").replace("-", "")
        for p in self.protocols.values():
            p_norm = p.code.upper().replace(" ", "").replace("-", "")
            if p_norm == code_norm:
                return p

        # Suffix match: "RCHOP21" matches "LYMPHOMA-RCHOP21"
        for p in self.protocols.values():
            p_norm = p.code.upper().replace(" ", "").replace("-", "")
            if p_norm.endswith(code_norm):
                return p
    ...
    def generate_protocol(self, request: ProtocolRequest) -> ProtocolResponse:
        ...
        # SAFETY CHECK 7: HBV reactivation risk with immunosuppressive agents
        ...
        # Find the actual HBV-risk drug name in this protocol for accurate warning text
        _hbv_drug_name = None
        for n in protocol_drug_names:
            for d in HBV_RISK_DRUGS:
                if d in n:
                    _hbv_drug_name = d.capitalize()
                    break
            if _hbv_drug_name:
                break
        if _hbv_drug_name:
            if patient.hep_b_surface_antigen == "positive":
                if not patient.hbv_prophylaxis_started:
                    warnings.append(Warning(
                        level="critical",
                        message=f"HBsAg POSITIVE: Fatal HBV reactivation risk with {_hbv_drug_name}. Antiviral prophylaxis (entecavir) MUST be started before {_hbv_drug_name} and continued for 12 months after last dose. Confirm prophylaxis is prescribed."
                    ))
                else:
                    warnings.append(Warning(
                        level="warning",
                        message=f"HBsAg positive — HBV prophylaxis confirmed started. Monitor HBV DNA every 3 months during and for 12 months after {_hbv_drug_name}."
                    ))
            elif patient.hep_b_core_antibody == "positive":
                if not patient.hbv_prophylaxis_started:
                    warnings.append(Warning(
                        level="warning",
                        message=f"Anti-HBc POSITIVE (HBsAg negative): Prior HBV exposure — reactivation risk with {_hbv_drug_name}. Antiviral prophylaxis or close HBV DNA monitoring required. Confirm management plan."
                    ))
                else:
                    warnings.append(Warning(
                        level="info",
                        message=f"Anti-HBc positive — HBV prophylaxis confirmed started. Monitor HBV DNA during {_hbv_drug_name} therapy."
                    ))
            elif patient.hep_b_surface_antigen is None or patient.hep_b_core_antibody is None:
                warnings.append(Warning(
                    level="warning",
                    message=f"HBV serology (HBsAg, Anti-HBc) not recorded — required before {_hbv_drug_name} due to fatal reactivation risk. Ensure screening is completed."
                ))
        ...
        if uses_anthracycline:
            ...
            if patient.lvef_percent is None:
                if has_cardiac_risk_factors:
                    warnings.append(Warning(
                        level="critical",
                        message="BASELINE ECHO REQUIRED: Patient has cardiac risk factors (age ≥70, cardiac history, or prior anthracyclines). Baseline LVEF must be documented before anthracycline administration."
                    ))
                else:
                    warnings.append(Warning(
                        level="warning",
                        message="BASELINE LVEF RECOMMENDED: Anthracycline-containing regimen — baseline echocardiogram recommended before starting treatment."
                    ))
        ...
        # SAFETY CHECK 14: Age-based suitability warnings
        protocol_code_upper = protocol.code.upper() if protocol.code else ""
        if patient.age_years >= 60:
            # BEACOPP-escalated: increased treatment-related mortality in >60
            if "BEACOPP" in protocol_code_upper:
                warnings.append(Warning(
                    level="critical",
                    message=f"PATIENT AGE {patient.age_years}: BEACOPP-escalated is generally NOT recommended for patients over 60 years due to significantly increased treatment-related mortality. Consider alternative regimen (e.g. ABVD, B-AVD). Proceed only after senior haematologist review."
                ))
            # High-dose cytarabine ≥60: dose reduction advisory
            uses_high_dose_cytarabine = any(
                "cytarabine" in d.drug_name.lower() and d.dose >= 1000
                for d in protocol.drugs
            )
            if uses_high_dose_cytarabine:
                warnings.append(Warning(
                    level="warning",
                    message=f"PATIENT AGE {patient.age_years} (≥60): High-dose cytarabine may require dose reduction to 1500 mg/m² or lower. Increased risk of cerebellar toxicity in older patients. Review with consultant."
                ))
        ...
        # Cisplatin renal safety net: warn at CrCl <60 (JSON dose mod rules handle actual reductions/omissions)
        if patient.creatinine_clearance is not None and patient.creatinine_clearance < 60:
            uses_cisplatin = any("cisplatin" in d.drug_name.lower() for d in protocol.drugs)
            if uses_cisplatin:
                warnings.append(Warning(
                    level="critical",
                    message=(
                        f"RENAL IMPAIRMENT: CrCl {patient.creatinine_clearance:.0f} ml/min. "
                        f"Cisplatin dose modification or omission applies per protocol rules. PRESCRIBER MUST REVIEW."
                    )
                ))
        ...
        if treatment_absolutely_contraindicated:
            ...
            # Sort drugs by administration_order so output matches protocol-specified sequencing
            sorted_cycle_drugs = sorted(cycle_drugs, key=lambda d: getattr(d, 'administration_order', 0))
            for drug in sorted_cycle_drugs:
        ...
        # Cascade: if cisplatin was in protocol but omitted from output, remove dependent pre/post meds
        _protocol_had_cisplatin = any("cisplatin" in d.drug_name.lower() for d in protocol.drugs)
        _cisplatin_in_output = any("cisplatin" in d.drug_name.lower() for d in chemo_doses)
        if _protocol_had_cisplatin and not _cisplatin_in_output:
            CISPLATIN_DEPENDENT_KW = ("hydration", "pre-hydration", "post-hydration",
                                      "furosemide", "aprepitant")
            for drug_list in (protocol.pre_medications, protocol.rescue_medications):
                for drug in drug_list:
                    d_name = drug.drug_name.lower()
                    d_id = (drug.drug_id or "").lower()
                    d_instr = (drug.special_instructions or "").lower()
                    if (any(kw in d_name or kw in d_id for kw in CISPLATIN_DEPENDENT_KW)
                            or "cisplatin" in d_instr or "cisplatin" in d_name):
                        contraindicated_drug_ids.add(drug.drug_id)
                        contraindicated_drug_ids.add(d_name)
        ...
        # Deduplicate warnings by message text (preserves order)
        _seen_msgs: set[str] = set()
        deduped_warnings: list[Warning] = []
        for w in warnings:
            if w.message not in _seen_msgs:
                _seen_msgs.add(w.message)
        ...
        return ProtocolResponse(
            ...
            treatment_delay_recommended=patient.requires_delay or getattr(patient, '_extra_delay', False),
            delay_reasons=list(dict.fromkeys((patient.delay_reasons if patient.requires_delay else []) + getattr(patient, '_extra_delay_reasons', []))),
    ...
    def _calculate_dose(
        ...
        if dose_unit == DoseUnit.MG_M2:
            ...
            calculated_dose = base_dose * bsa
            final_unit = "mcg"
        elif dose_unit == DoseUnit.MCG_KG:
            calculated_dose = base_dose * patient.weight_kg
            final_unit = "mcg"
        else:
            # Flat dose
        ...
        for rule in modification_rules:
            if _drug_matches_rule(drug.drug_id, drug.drug_name, rule.affected_drugs):
                ...
                if mod_applied:
                    ...
                    # Handle "delay" type rules — treatment should be held
                    rule_mod_type = (rule.modification_type or "").lower()
                    if rule_mod_type == "delay":
                        warnings.append(Warning(
                            level="critical",
                            message=f"TREATMENT HOLD: {mod_desc}",
                            drug_id=drug.drug_id
                        ))
                        modifications.append(f"{drug.drug_name}: HOLD — {mod_desc}")
                        # Signal delay via patient object
                        if not patient.requires_delay:
                            patient._extra_delay = True
                            if not hasattr(patient, '_extra_delay_reasons'):
                                patient._extra_delay_reasons = []
                            patient._extra_delay_reasons.append(mod_desc)
        ...
        # VINCRISTINE NEUROPATHY GRADE 2: Override to flat 1mg dose per NHS protocol
        # The percentage-based rule system cannot express "set to flat 1mg" so we handle it here.
        if (
            "vincristine" in drug.drug_name.lower()
            and patient.peripheral_neuropathy_grade is not None
            and patient.peripheral_neuropathy_grade == 2
            and patient.has_peripheral_neuropathy
        ):
            pre_neuro_dose = calculated_dose
            calculated_dose = 1.0
            dose_modified = True
            modification_reason = "Peripheral neuropathy grade 2: vincristine reduced to flat 1mg dose per protocol"
            modification_percent = None
            modifications.append(f"{drug.drug_name}: reduced to 1mg flat dose (grade 2 neuropathy)")
            warnings.append(Warning(
                level="warning",
                message=(
                    f"VINCRISTINE NEUROPATHY GRADE 2: Dose reduced to 1mg flat dose "
                    f"(was {pre_neuro_dose:.1f}mg). "
                    f"Permanently stop vincristine if toxicity worsens despite reduction."
                ),
                drug_id=drug.drug_id
            ))
    ...
    def _apply_modification_rule(
        ...
        param_map = {
            ...
            # Neuropathy parameters — allow dose modification rules to fire
            "peripheral_neuropathy_grade": patient.peripheral_neuropathy_grade,
            "nci_ctc_motor_neuropathy": patient.peripheral_neuropathy_grade,
            "nci_ctc_sensory_neuropathy": patient.peripheral_neuropathy_grade,
            "nci_ctc_peripheral_neuropathy": patient.peripheral_neuropathy_grade,
        }
        ...
        primary_met = evaluate_condition(value, rule)
        ...
        if rule.secondary_parameter and rule.secondary_connector:
            ...
            if sec_value is None:
                # Secondary lab not provided — cannot evaluate
                sec_met = False
            else:
            ...
            if rule.secondary_connector == "AND":
                # Both must be met
                if not (primary_met and sec_met):
                    return False, 1.0, ""
            elif rule.secondary_connector == "OR":
                # Either must be met
                if not (primary_met or sec_met):
                    return False, 1.0, ""
        else:
            # No secondary — just use primary
            if not primary_met:
                return False, 1.0, ""
                ...
                if hepatic_drugs:
                    warnings.append(Warning(
                        ...
                        message=(
                            ...
                            f"Hepatically-metabolised drugs in this protocol: {', '.join(hepatic_drugs)}. "
                            f"Review individual drug SPCs — dose reductions or omissions may be required. "
                    ...
                    warnings.append(Warning(
                        ...
                        message=(
                            ...
                            f"Review hepatic function before proceeding."
                        )
                    ))
            elif bili > 51:
                if hepatic_drugs:
                    warnings.append(Warning(
                        ...
                        message=(
                            ...
                            f"Dose reductions may be required for: {', '.join(hepatic_drugs)}. "
            ...
            if crcl < 30:
                warnings.append(Warning(
                    ...
                    message=(
                        ...
                        f"{'Cisplatin should be OMITTED at CrCl <40 ml/min per protocol. ' if any('cisplatin' in d.lower() for d in renal_drugs) else ''}"
                ...
                # Cisplatin at CrCl 40-59: dose reduction per protocol rules
                if any('cisplatin' in d.lower() for d in renal_drugs):
                    pass  # Handled by SAFETY CHECK in main generate + per-protocol dose mod rules
