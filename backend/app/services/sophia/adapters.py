...
from app.services.sophia.models import PatientData, ECOGPerformanceStatus, LabUnitSystem
...
class PatientStateAdapter:
    ...
    def adapt(data: Dict[str, Any], target_cycle: Optional[int] = None) -> PatientData:
        ...
        # Safety fields (pass-through from ChemoCare)
        if data.get("active_infection") is not None:
            kwargs["active_infection"] = bool(data["active_infection"])
        if data.get("pregnancy_status"):
            kwargs["pregnancy_status"] = data["pregnancy_status"]
        if data.get("tls_risk"):
            kwargs["tls_risk"] = data["tls_risk"]
        if data.get("lvef_percent") is not None:
            kwargs["lvef_percent"] = float(data["lvef_percent"])
        if data.get("peripheral_neuropathy_grade") is not None:
            kwargs["peripheral_neuropathy_grade"] = int(data["peripheral_neuropathy_grade"])
        if data.get("prior_cardiac_history") is not None:
            kwargs["prior_cardiac_history"] = bool(data["prior_cardiac_history"])
        if data.get("prior_anthracycline_dose_mg_m2") is not None:
            val = opt_float(data["prior_anthracycline_dose_mg_m2"])
            if val is not None:
                kwargs["prior_anthracycline_dose_mg_m2"] = val
