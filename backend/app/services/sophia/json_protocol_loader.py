...
CURATED_DIR = Path(__file__).parent / "data" / "protocols"
...
def load_curated_protocols() -> dict[str, Protocol]:
    """
    Load NHS-curated protocols from data/protocols/ (rich schema, direct Protocol model).
    These override anything from protocol_jsons_normalized/ for the same protocol_code.
    """
    ...
    for json_file in sorted(CURATED_DIR.glob("*.json")):
        if json_file.name == "protocol_index.json":
            continue
        try:
            with open(json_file, "r", encoding="utf-8") as f:
                raw = json.load(f)
            # Files may be a single dict or a list
            ...
            for p in entries:
                ...
                try:
                    protocol = Protocol(**p)
                    code = protocol.code or p.get("protocol_code", "")
                    if code:
                        result[code] = protocol
                except Exception as e:
                    print(f"[JSON LOADER] Curated parse error in {json_file.name}: {e}")
        except Exception as e:
            print(f"[JSON LOADER] Error reading curated file {json_file.name}: {e}")

    print(f"[JSON LOADER] Loaded {len(result)} curated protocols from data/protocols/")
    return result


def load_normalized_protocols() -> dict[str, Protocol]:
    """
    Load protocols from protocol_jsons_normalized/ directory.
    ...
    print(f"[JSON LOADER] Loaded {len(result)} protocols from protocol_jsons_normalized/")
    return result


def load_all_json_protocols() -> dict[str, Protocol]:
    """
    Load only curated protocols from data/protocols/.
    These are the 15 production-quality, clinician-verified protocols.
    Returns dict keyed by protocol_code.
    """
    return load_curated_protocols()
